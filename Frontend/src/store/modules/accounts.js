...
const actions = {
  ...
  postSignInData(context, signInData) {
    axios.get('https://http://i5a404.p.ssafy.io/auth/login', { params :{ id: signInData.id, password: signInData.password } })
      .then(res => {
  ...
  postSignUpData(context, signUpData) {
    axios.post('https://http://i5a404.p.ssafy.io/auth/register', signUpData)
      .then(res => {
        // 여기서 할게 없음
        // 회원가입이 성공적으로 되었다 메세지?
        console.log(res)
